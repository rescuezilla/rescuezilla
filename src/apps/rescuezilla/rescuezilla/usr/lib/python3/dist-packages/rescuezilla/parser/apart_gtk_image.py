...
class ApartGtkImage:
    def __init__(self, absolute_apart_gtk_img_path):
        ...
        self.enduser_filename = os.path.join(
            os.path.basename(os.path.dirname(self.absolute_path)), "*.apt.*"
        )
        self.user_notes = ""
        self.warning_dict = {}
        ...
        for image_absolute_path in image_absolute_path_list:
            ...
            node_with_timestamp = (
                metadata_dict["name"] + "-" + metadata_dict["timestamp"]
            )
            if metadata_dict["inprogress"]:
                self.warning_dict[image_absolute_path] = (
                    "Partition image appears to be partially complete (or still in progress)"
                )
                continue
            else:
                ...
                with Utility.setlocale("C"):
                    ...
                    metadata_dict["timestamp"] = datetime.strptime(
                        metadata_dict["timestamp"], "%Y-%m-%dT%H%M"
                    )
            ...
            if metadata_dict["filesystem"] != "dd":
                self.image_format_dict_dict[node_with_timestamp] = {
                    "type": "partclone",
                    "absolute_filename_glob_list": [image_absolute_path],
                    "compression": ApartGtkImage._convert_compression_identifier(
                        metadata_dict["compression"]
                    ),
        ...
        self.last_modified_timestamp = format_datetime(
            datetime.fromtimestamp(max_timestamp.timestamp())
        )
        print(
            "Last modified timestamp (max timestamp): " + self.last_modified_timestamp
        )
    ...
    def flatten_partition_string(self, key):
        return (
            self.image_format_dict_dict[key]["filesystem"]
            + " "
            + Utility.human_readable_filesize(
                self._compute_partition_size_byte_estimate(key)
            )
        )
    ...
    def _parse_filename(apart_gtk_filename):
        ...
        if not m.match(
            r"^([a-zA-Z0-9-_+]*)-([0-9]+-[0-9]+-[0-9]+T[0-9]+)\.apt\.([a-zA-Z0-9+]+)\.([a-zA-Z0-9]+)([\.inprogress]?)"
        ):
    ...
    def _compute_partition_size_byte_estimate(self, key):
        ...
        for absolute_filename in self.image_format_dict_dict[key][
    ...
    def _convert_compression_identifier(filename_string):
        ...
        # "also support .zstd from v0.14", so Rescuezilla matches this by checking for both "zst" and "zstd".
            ...
            raise Exception(
                "Unable to query Apart GTK's file compression: " + filename_string
            )
