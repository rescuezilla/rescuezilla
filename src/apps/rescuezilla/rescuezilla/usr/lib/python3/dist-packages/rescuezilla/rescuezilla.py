...
def main():
    ...
    print(
        "WARNING: Rescuezilla's command-line interface should be considered as EXPERIMENTAL and UNSTABLE"
    )
    print(
        "WARNING: Command-line interface behavior may change between versions without notice."
    )
    ...
    if args.command is None:
        ...
        cli_mode(args=args, human_readable_version=human_readable_version)


def cli_mode(args: argparse.Namespace, human_readable_version: str):
    print(str(args))

    cli_ui_manager = UiManager()
    if args.command == "backup":
        backup_manager = BackupManager(
            ui_manager=cli_ui_manager, human_readable_version=human_readable_version
        )
        ...
        backup_manager.start_backup(
            selected_drive_key=args.source,
        ...
        restore_mapping_dict = PartitionsToRestore.get_partition_to_restore(
            selected_image=image,
            dest_drive_node=args.destination,
            partitions_to_restore=args.partitions,
        )
        restore_manager.start_restore(
            image=image,
        ...
        # Get the partitions in the image, and the partitions on the destination disk, with the assumption the
        # partition table is being overwritten
        clone_mapping_dict = PartitionsToRestore.get_partition_to_restore(
            selected_image=metadata_img,
            dest_drive_node=args.destination,
            partitions_to_restore=args.partitions,
        )
        clone_manager.start_clone(
            image=metadata_img,
...
def get_metadata_only_img(source_drive_key: str):
    ...
    if len(source_drive_metadata_only_image.warning_dict) > 0:
        ...
        for value in source_drive_metadata_only_image.warning_dict.values():
            error_msg += value + "\n"
            print(
                ("Unable to process {source}:").format(source=source_drive_key)
                + "\n\n"
                + error_msg
            )
