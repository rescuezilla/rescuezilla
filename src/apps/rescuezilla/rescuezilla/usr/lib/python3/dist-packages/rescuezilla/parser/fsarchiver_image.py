...
class FsArchiverImage:
    def __init__(self, absolute_fsarchiver_fsa_path, enduser_filename, filename):
        ...
        if os.path.exists(qt_fsarchiver_notes_abs_path):
            self.user_notes = Utility.read_file_into_string(
                qt_fsarchiver_notes_abs_path
            )

        # Use `fsarchiver archinfo <fsa path>` to determine if the archive is encrypted
        self.is_needs_decryption = False
        dirname = os.path.dirname(absolute_fsarchiver_fsa_path)
        fsarchiver_archinfo_cmd_list = [
            "fsarchiver",
            "archinfo",
            absolute_fsarchiver_fsa_path,
        ]
        process, flat_command_string, fail_description = Utility.run(
            "fsarchiver archinfo", fsarchiver_archinfo_cmd_list, use_c_locale=True
        )
        if process.returncode == 0:
            # Process the archive information
            self.fsa_dict = FsArchiverImage.parse_fsarchiver_archinfo_output(
                process.stderr
            )
            print("Processed: " + str(self.fsa_dict))
        else:
            self.last_modified_timestamp = format_datetime(
                datetime.fromtimestamp((os.stat(absolute_fsarchiver_fsa_path).st_mtime))
            )
            self.size_bytes = Utility.count_total_size_of_files_on_disk(
                [absolute_fsarchiver_fsa_path], "unknown"
            )
            ...
            self.warning_dict["archinfo"] = (
                "Unable to process FSArchiver" + process.stdout + " " + process.stderr
            )
            return
        ...
        with Utility.setlocale("C"):
            ...
            fsarchiver_datetime = datetime.strptime(
                self.fsa_dict["date"], "%Y-%m-%d_%H-%M-%S"
            )
        # Convert to a string that's consistent with the rest of Rescuezilla
        self.last_modified_timestamp = format_datetime(
            datetime.fromtimestamp(fsarchiver_datetime.timestamp())
        )
    ...
    def get_enduser_friendly_partition_description(self):
        ...
        for fs_key in self.fsa_dict["filesystems"].keys():
            flat_string += (
                "(" + fs_key + ": " + self.flatten_partition_string(fs_key) + ") "
            )
            index += 1
        return flat_string
            ...
            for line in block_string.splitlines():
                ...
                try:
                    ...
                    if key == "Filesystem id in archive":
                        ...
                        fsarchiver_archinfo_dict["filesystems"][fs_key][
                            "size_bytes"
                        ] = FsArchiverImage.extract_bytes_from_string(value)
