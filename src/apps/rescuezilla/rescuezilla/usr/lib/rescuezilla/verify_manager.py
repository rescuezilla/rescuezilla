...
class VerifyManager:
    ...
    def start_verify(
        self, image_list, completed_callback, on_separate_thread: bool = True
    ):
    ...
    def do_verify_wrapper(self):
        try:
            ...
            return self.ui_manager.completed_operation(
                callable_fn=self.completed_verify,
                succeeded=False,
    ...
    def do_verify(self):
        ...
        for image in self.image_list:
            ...
            self.ui_manager.display_status(
                msg1=image_verify_message, msg2=image_verify_message
            )
            ...
            if isinstance(image, FsArchiverImage):
                with self.summary_message_lock:
                    self.summary_message += (
                        _("⚠") + " " + "Verifying FsArchiver images not yet supported\n"
                    )
                continue
            if isinstance(image, MetadataOnlyImage):
                with self.summary_message_lock:
                    self.summary_message += (
                        _("⚠") + " " + "Verifying VM images not yet supported\n"
                    )
                continue
            ...
            if image.has_partition_table():
                ...
                if "absolute_path" in image.post_mbr_gap_dict.keys():
                    post_mbr_size += int(
                        os.stat(image.post_mbr_gap_dict["absolute_path"]).st_size
                    )
                ...
                if (mbr_size + post_mbr_size) <= 512:
                    ...
                    self.summary_message += (
                        _("✔") + " " + _("MBR backup appears correct.") + "\n"
                    )
            else:
            ...
            if image.normalized_sfdisk_dict["file_length"] == 0:
                self.summary_message += (
                    _("❌")
                    + " "
                    + _("Sfdisk partition table file is empty or missing.")
                    + "\n"
                )
            else:
                self.summary_message += (
                    _("✔") + " " + _("Sfdisk partition table file is present.") + "\n"
                )
            ...
            for partition_key in image.image_format_dict_dict.keys():
                ...
                if (
                    ...
                    in image.image_format_dict_dict[partition_key].keys()
                ):
                    ...
                    partition_estimated_size_bytes = (
                        Utility.count_total_size_of_files_on_disk(
                ...
                self.logger.write(image_verify_message)
                self.ui_manager.display_status(
                    msg1=image_verify_message + " " + filesystem_verify_message,
                    msg2=filesystem_verify_message,
                )
                ...
                if "type" in image.image_format_dict_dict[partition_key].keys():
                    ...
                    if image_type == "swap":
                        self.summary_message += (
                            ...
                            + " "
                            + partition_key
                            + ": verifying swap partition images not yet supported.\n"
                        )
                        cumulative_bytes += partition_estimated_size_bytes
                        continue
                    if image_type == "missing":
                        self.summary_message += (
                            _("❌") + " " + partition_key + ": partition is missing.\n"
                        )
                        cumulative_bytes += partition_estimated_size_bytes
                        continue
                    if (
                        "dd" == image_type
                        or image.image_format_dict_dict[partition_key]["binary"]
                        == "partclone.dd"
                    ):
                        self.summary_message += (
                            _("⚠")
                            + " "
                            + partition_key
                            + ": verifying raw dd images not yet supported.\n"
                        )
                        cumulative_bytes += partition_estimated_size_bytes
                        continue
                        ...
                        self.summary_message += (
                            ...
                            + " "
                            + partition_key
                            + ": verifying PartImage images not yet supported.\n"
                        )
                        cumulative_bytes += partition_estimated_size_bytes
                        continue
                        ...
                        self.summary_message += (
                            ...
                            + " "
                            + partition_key
                            + ": Verifying NTFSclone images not yet supported.\n"
                        )
                        cumulative_bytes += partition_estimated_size_bytes
                        continue
                    elif "unknown" != image_type:
                        self.summary_message += (
                            _("❌") + " " + partition_key + ": unknown image type.\n"
                        )
                        cumulative_bytes += partition_estimated_size_bytes
                        continue
                    else:
                        message = (
                            "Unhandled type" + image_type + " from " + partition_key
                        )
                    ...
                    flat_command_string = Utility.print_cli_friendly(
                        image_type + " command ",
                        [cat_cmd_list, decompression_cmd_list, verify_command_list],
                    )
                    ...
                    verify_decompression_proc_key = (
                        "decompression_" + image.absolute_path + "_" + partition_key
                    )
                    self.proc[verify_decompression_proc_key] = subprocess.Popen(
                        ...
                        if output and ("partclone" == image_type or "dd" == image_type):
                            ...
                            if "remaining" in temp_dict.keys():
                                self.ui_manager.update_progress_status(
                                    message=filesystem_verify_message + "\n\n" + output
                                )
                        elif "partimage" == image_type:
                            self.ui_manager.display_status(
                                msg1="partimage: " + filesystem_verify_message, msg2=""
                            )
                        elif "ntfsclone" == image_type:
                            self.ui_manager.display_status(
                                msg1="ntfsclone: " + filesystem_verify_message, msg2=""
                            )
        ...
        return self.ui_manager.completed_operation(
            callable_fn=self.completed_verify, succeeded=True, message=""
        )
    ...
    def completed_verify(self, succeeded, message):
        ...
        duration_minutes = Utility.get_human_readable_minutes_seconds(
            (verify_timeend - self.verify_timestart).total_seconds()
        )
...
{message}""").format(
                heading=_("Verification Summary"),
