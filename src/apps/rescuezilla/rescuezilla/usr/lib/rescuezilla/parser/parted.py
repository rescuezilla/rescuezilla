...
class Parted:
    ...
    def parse_parted_output(parted_output):
        ...
        for metadata_line in metadata_list:
            ...
            # Processing: Sector size (logical/physical): 512B/512B
                ...
                r"Sector size .logical/physical.: ([0-9]+)[a-zA-Z]+/([0-9]+)[a-zA-Z]+"
            ):
            ...
            for partition_line in partition_list:
                ...
                if parted_dict["partition_table"] == "gpt":
                    ...
                    partition_number = int(
                        partition_line[0 : column_title.index("Start") - 1].strip()
                    )
                    parted_dict["partitions"][partition_number] = (
                        collections.OrderedDict()
                    )
                    # print("Looking at line " + partition_line)
                    parted_dict["partitions"][partition_number]["start"] = int(
                        partition_line[
                            column_title.index("Start") : column_title.index("End") - 1
                        ].strip()[:-1]
                    )
                    parted_dict["partitions"][partition_number]["end"] = int(
                        partition_line[
                            column_title.index("End") : column_title.index("Size") - 1
                        ].strip()[:-1]
                    )
                    parted_dict["partitions"][partition_number]["size"] = int(
                        partition_line[
                            column_title.index("Size") : column_title.index(
                                "File system"
                            )
                            - 1
                        ].strip()[:-1]
                    )
                    parted_dict["partitions"][partition_number]["filesystem"] = (
                        partition_line[
                            column_title.index("File system") : column_title.index(
                                "Name"
                            )
                            - 1
                        ].strip()
                    )
                    parted_dict["partitions"][partition_number]["name"] = (
                        partition_line[
                            column_title.index("Name") : column_title.index("Flags") - 1
                        ].strip()
                    )
                    parted_dict["partitions"][partition_number]["flags"] = (
                        partition_line[
                            column_title.index("Flags") : len(partition_line)
                        ].strip()
                    )
                ...
                #  1      1048576B    91226111B    90177536B    primary   ext4
                    ...
                    parted_dict["partitions"][partition_number]["size"] = int(
                        partition_line[
                            column_title.index("Size") : column_title.index("Type") - 1
                        ].strip()[:-1]
                    )
                    parted_dict["partitions"][partition_number]["type"] = (
                        partition_line[
                            column_title.index("Type") : column_title.index(
                                "File system"
                            )
                            - 1
                        ].strip()
    ...
    def has_flag(parted_dict, partition_key, flag):
        if (
            partition_key in parted_dict["partitions"].keys()
            and "flags" in parted_dict["partitions"][partition_key].keys()
