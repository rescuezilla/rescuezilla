...
use constant APP_ICON        => DATA_DIR."icon.png";
...
sub do_backup {
  ...
  if ($status{'mount_type'} =~ /SMB/) {
      # Display a fatal error if the "ESTALE" SMB/CIFS issue is detected, with a clear message explaining the issue.
      my $estale_bug_message = "Failed to write test file $dest/$file.test.file into the SMB mount directory. This may occur when using a more recent SMB server. The root cause is being investigated.\n\nAs a workaround, please select a local destination or an alternative SMB server.\n\nSee https://github.com/rescuezilla/rescuezilla/issues/84 for more information.";
      # This open() function is not processing the path through a shell, so no need to use shell_quote().
      open(OUT, ">$dest/$file.test.file") or fatal_crash($estale_bug_message);
      close OUT;
      system("rm $quoted_tmp_sanity_check");
  }
...
sub mount_data {
  # Mounts the specified local disk, SMB/CIFS server
...
sub set_share_dropdown {  
  ...
  $mdl->set($mdl->append, 0, "SMB|", 1, loc("SMB/CIFS shared folder specified below"));
  $cbo->set_active(0);
}
...
sub find_shared_drives {
  # Search for local network shares (SMB/CIFS)
  our %shares;
