...
if len(sys.argv) < 3:
    raise Exception(
        "Usage: ./replace_english_translation_string.py search_string replace_string"
    )
    sys.exit(1)
...
language_glob = os.path.join(
    git_root.strip(), "src/apps/rescuezilla/rescuezilla/usr/share/locale/*"
)
...
subprocess.run(
    [
        "sed",
        "-i",
        "s*" + escaped_search_str + "*" + escaped_replace_str + "*g",
        "src/apps/rescuezilla/rescuezilla/usr/share/rescuezilla/rescuezilla.glade",
    ],
    capture_output=True,
)
...
for language_dir in language_dir_list:
    ...
    subprocess.run(
        ["sed", "-i", "s*" + search_str + "*" + replace_str + "*g", language_file],
        capture_output=True,
    )
